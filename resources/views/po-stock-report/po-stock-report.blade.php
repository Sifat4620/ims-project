                    ...
                    <tbody>
                        @forelse ($paginatedItems as $item)
                            <tr>
                                ...
                                <td rowspan="2">{{ $item['condition'] }}</td>
                                <td rowspan="2">{{ $item['holding_location'] }}</td>
                                <td rowspan="2">{{ $item['date'] }}</td>
                                <td rowspan="2">
                                    <button class="btn btn-sm btn-primary shadow-sm" onclick="previewReport(@json($item))">
    ...
    const content = `
        ...
        <table border="1" style="width: 100%; border-collapse: collapse; text-align: left;">
            ...
            <tbody>
                ...
                <tr><td>Serial Numbers (Yes)</td><td>${(item.serial_numbers_yes || []).join(', ')}</td></tr>
                <tr><td>Serial Numbers (No)</td><td>${(item.serial_numbers_no || []).join(', ')}</td></tr>
                <tr><td>Total Qty</td><td>${(item.serial_numbers_yes?.length || 0) + (item.serial_numbers_no?.length || 0)}</td></tr>
